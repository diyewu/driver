package xz.research.common.service.impl;
...
public class RedisServiceImpl implements RedisService {
    ...
    @Override
    public String setMap(String key, Map<String,String> value) throws RedisConnectException {
        return this.excuteByJedis(j -> j.hmset(key.toLowerCase(), value));
    }


    @Override
    public  Set<String> getMapKeys(String key) throws RedisConnectException {
        return this.excuteByJedis(j -> j.hkeys(key.toLowerCase()));
    }

    @Override
    public boolean mapExistsKey(String key,String mapKey) throws RedisConnectException{
        return this.excuteByJedis(j -> j.hexists(key.toLowerCase(),mapKey));
    }


    @Override
    public Long hset(String key, String mapKey, RcRegion r) throws RedisConnectException{
        ObjectMapper mapper = new ObjectMapper();
        return this.excuteByJedis(j -> j.hset(key.toLowerCase(), mapKey, mapper.writeValueAsString(r)));
    }

    @Override
    public RcRegion hget(String key, String mapKey) throws RedisConnectException, IOException {
        ObjectMapper mapper = new ObjectMapper();
        String s = this.excuteByJedis(j -> j.hget(key, mapKey));
        RcRegion rcRegion = mapper.readValue(s, RcRegion.class);
        return rcRegion;
    }
